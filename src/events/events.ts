...
export class EventManager<T extends EventRegistry> {
	...
	on(
		key: string,
		callback: (value: any, event: EventContext) => void,
		options?: ListenerOptions,
	): () => void;
	...
	off(key: string, callback: (value: any, event: EventContext) => void) {
		const list = this.listeners[key];
		if (!list) return;
		(this.listeners[key] as any) = list.filter(
	...
	emit<K extends keyof T>(
		key: K,
		value: T[K],
		defaultCallback?: EventCallback<T[K]>,
	): EventContext;
	emit(
		key: string,
		value: any,
		defaultCallback?: EventCallback<any>,
	): EventContext;
	...
	emit(
		...
		if (!current || current.length === 0) {
			...
			if (!emptyCtx.defaultPrevented) defaultCallback?.(value, emptyCtx);
			return emptyCtx;
		}
		...
		if (!ctx.defaultPrevented) defaultCallback?.(value, ctx);
